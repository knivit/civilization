...
public class CivilizationUnhappinessService {
    ...
    private static final Map<MapSize, UnhappinessPerCity> UNHAPPINESS_PER_CITY_FOR_WORLD_SIZE = Map.of(
        ...
        STANDARD, new UnhappinessPerCity(3.0, 5.0),
        LARGE, new UnhappinessPerCity(2.4, 4.0),
        HUGE, new UnhappinessPerCity(1.8, 3.0)
    );

    private static final Map<DifficultyLevel, Double> UNHAPPINESS_PER_CITY_FOR_DIFFICULT_LEVEL = Map.of(
        SETTLER, 0.4,
        CHIEFTAIN, 0.6,
        WARLORD, 0.75,
        PRINCE, 1.0,
    ...
    public Unhappiness calc() {
        ...
        int inMyCities = (int)civilization.getCityService().stream()
            .filter(e -> !e.isAnnexed())
            .count();

        int inAnnexedCities = (int)civilization.getCityService().stream()
            .filter(City::isAnnexed)
            .count();
