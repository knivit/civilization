...
public class HexagonRender {
    ...
    public void render(RenderContext context, GraphicsContext graphicsContext, int x, int y, Color color) {
        graphicsContext.getG().setColor(color);

        float[] ox = context.getHexBordersX();
        float[] oy = context.getHexBordersY();
        graphicsContext.getG().fillPolygon(
