 ...
 */
public class TileMap implements IsPartOfGameInfoSerialization {
    /** Legacy way to store starting locations - now this is used only in [translateStartingLocationsFromMap] */
    private static final startingLocationPrefix = "StartingLocation ";
    ...
    public TileMap(int initialCapacity) {
        tileList = new ArrayList<>(initialCapacity);
    }
    ...
    private List<Vector2> startingLocations = arrayListOf(StartingLocation(Vector2.Zero, legacyMarker));
    ...
    Map tileUniqueMapCache = new ConcurrentHashMap<List<String>, UniqueMap>();

    List<List<Tile>> tileMatrix = new ArrayList<>(); // this works several times faster than a hashmap, the performance difference is really astounding
    ...
    float maxLatitude() {
        ...
        else return values.maxOf { abs(it.latitude) }
    }

    float maxLongitude() {
        if (values.isEmpty()) return 0f;
        else return values.maxOf { abs(it.longitude) };
    }

    Set<String> naturalWonders() {
        return tileList.asSequence().filter { it.isNaturalWonder() }.map { it.naturalWonder!! }.toSet();
    }

    Set<String> resources() {
        return tileList.asSequence().filter { it.resource != null }.map { it.resource!! }.toSet();
    }
    ...
    Map startingLocationsByNation = new HashMap<String, HashSet<Tile>>();

    /** Continent ID to Continent size */
    Map continentSizes = new HashMap<Integer, Integer>();
     ...
     * */
    public TileMap(int radius, RuleSet ruleset, Boolean worldWrap: Boolean = false)
            : this (HexMath.getNumberOfTilesInHexagon(radius)) {
        startingLocations.clear();
        String firstAvailableLandTerrain = MapLandmassGenerator.getInitializationTerrain(ruleset, TerrainType.Land);
        for (Vector2 vector : HexMath.getVectorsInDistance(Vector2.Zero, radius, worldWrap)) {
            Tile tile = new Tile();
            tile.position = vector;
            tile.baseTerraint = firstAvailableLandTerrain;
            tileList.add(tile);
        }
    ...
    public TileMap(int width, int height, Ruleset ruleset, worldWrap: Boolean = false)
            : this(width * height) {
        startingLocations.clear();
        ...
        for (column in -wrapAdjustedWidth / 2 .. (wrapAdjustedWidth-1) / 2){
            for (row in -height / 2 ..(height - 1) / 2) {
                Tile tile = new Tile();
                tile.position = HexMath.getTileCoordsFromColumnRow(column, row);
                tile.baseTerrain = firstAvailableLandTerrain;
                tileList.add(tile);
            }
        }
    ...
    public TileMap clone() {
        TileMap toReturn = new TileMap(tileList.size());
    ...
    public Tile get(int x, int y) {
        return tileMatrix.get(x - leftX).get(y - bottomY);
    }
    ...
    private Tile getOrNull(int x, int y) {
        return tileMatrix.getOrNull(x - leftX) ?.getOrNull(y - bottomY);
    }
    ...
    public List<Tile> getTilesInDistance(Vector2 origin, int distance) {
     ...
     *  Respects map edges and world wrap. */
    public List<Tile> getTilesInDistanceRange(Vector2 origin, int from, int to) {
        List<Tile> result = new ArrayList<>();
        for (int i = from; i < to; i ++) {
            result.addAll(getTilesAtDistance(origin, i));
        }
        return result;
    }
     ...
     *  Respects map edges and world wrap. */
    public List<Tile> getTilesAtDistance(Vector2 origin, int distance) {
     ...
     */
    public int getNeighborTileClockPosition(Tile tile, Tile otherTile) {
        int radius = 0;
        if (mapParameters.shape == MapShape.rectangular) radius = mapParameters.mapSize.width / 2;
        else radius = mapParameters.mapSize.radius;
        int x1 = (int)tile.position.x;
        int y1 = (int)tile.position.y;
        int x2 = (int)otherTile.position.x;
        int y2 = (int)otherTile.position.y;
        ...
        if (xDifference == 1 && yDifference == 1) return 6;             // otherTile is below
        if (xDifference == -1 && yDifference == -1) return 12;          // otherTile is above
        if (xDifference == 1 || xWrapDifferenceBottom == 1) return 4;   // otherTile is bottom-right
        if (yDifference == 1 || yWrapDifferenceBottom == 1) return 8;   // otherTile is bottom-left
        if (xDifference == -1 || xWrapDifferenceTop == -1) return 10;   // otherTile is top-left
        if (yDifference == -1 || yWrapDifferenceTop == -1) return 2;    // otherTile is top-right
        return -1;
    }
     ...
     */
    public Tile getClockPositionNeighborTile(Tile tile, int clockPosition) {
     ...
     */
    public Vector2 getNeighborTilePositionAsWorldCoords(Tile tile, Tile otherTile) {
        return HexMath.getClockPositionToWorldVector(getNeighborTileClockPosition(tile, otherTile));
    }
     ...
     */
    public Vector2 getUnWrappedPosition(Vector2 position) {
